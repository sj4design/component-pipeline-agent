...
description: "Run full component pipeline: research → spec → enrich → generate → figma-qa"
---

# /build $ARGUMENTS

Full pipeline orchestrator. Read `build-agent/SKILL.md` first, then execute.

Runs: research → spec → enrich → generate → figma-qa in ONE pass.

Flags: --from=spec (skip research), --to=enrich (stop early), --dry (no Figma), --human (generate .md files), --fresh (ignore cache), --max (no questions, ALL patterns)
